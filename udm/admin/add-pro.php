...
if(isset($_POST['submit'])){
	...
	$sql=mysqli_query($conn,"select id from image where pname='$product_name'");
	...
	if($row>0)
{
	...
	if(move_uploaded_file($_FILES["image"]["tmp_name"],'../images/'.$filename)){
		// Image db insert sql
		$insert = "INSERT into image(pname,file_name,uploaded_on,status,description,qty) values('$product_name','$filename',now(),1,'$description','$quantity')";
...
<body>
	...
	<div class="form-group">
		<label for="pro_qty" class="col-sm-3 col-form-label">Quantity</label>
	 <input type="number" name="qty" id="pname" >
	</div>
